...
void IRAM_ATTR P3RGB64x32MatrixPanel::draw() {
  ...
  for (int x = 0; x < 64; x++) {
    ...
    REG_WRITE(GPIO_OUT_REG, out | ((uint32_t)r1 << pinR1) | ((uint32_t)g1 << pinG1) | ((uint32_t)b1 << pinB1) | ((uint32_t)r2 << pinR2) | ((uint32_t)g2 << pinG2) | ((uint32_t)b2 << pinB2));
  ...
  out = ((y & 1) << pinA) | ((y & 2) << (pinB - 1)) | ((y & 4) << (pinC - 2)) | ((y & 8) << (pinD - 3));
