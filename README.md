...
If the program is in recording mode (it is saving the audio signals into an audio file), the audio data shall be sent to a web server for storage through Raspberry PI.

When the program is playing a saved audio file, the audio data shall be sent from the web to the AtMega328PB via the Raspberry PI chip, where its amplitude and frequency can be outputted to the speaker pin via a sine wave to play the audio.

The ESP32 shall have a program that processes the output sine wave from the AtMega328PB and convert it into LED data that can be used for audio visualization.

### 5. Hardware Requirements Specification (HRS)

#### Overview

The hardware components should be able to convert audio signals into electrical signals and electrical signals into audio signals. The hardware should also include LED lights so the user can visualize the audio data. Finally, the hardware should include interactive components such as the remote control and a potentiometer so the users can adjust volume, select audio files, and choose when to start and stop recording.
...
The Raspberry PI shall be used to communicate with a web server to read audio data.

A remote control shall be used to select different songs and actions.

### 6. MVP Demo

What do you expect to accomplish by the first milestone?

By the first milestone, we expect to have all the necessary components ordered. This includes the microphone, speaker, LED strips, and other components like a knob for volume control. We also expect to have implemented real-time audio visualization and amplification by the first milestone. In other words, when music is played into the device's microphone, the device should be able to output the audio signals through the LED strips and the speaker. We also hope to have implemented adjustable volume control via a potentiometer knob. Also, we expect to have obtained and properly modified a container that can house all the circuitry and sensors as well as the knobs and buttons for volume control, recording music, selecting music files, etc..
