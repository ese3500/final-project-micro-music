...
"The program shall take input audio data via UART from a device that can sample audio from a microphone at a rate of 44.1 kHz. We can test this using serial print to a computer to make sure the right data is being sent." We realized that audio visualization doesn't need to be sampled at 44.1 kHz for a sound that is dominantly less than 7.5KH. As a result, the program simply used the AtMega328PB's onboard ADC to sample at 15 kHz, which is suitable for audio visualization
