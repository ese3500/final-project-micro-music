...
The software will process the input audio signals from the microphone and ouput the signals through the LED strips and the speaker. It should also be able to store the audio data using a web server. Finally, the software should be able to write to an LCD screen and take user input via buttons and potentiometers so the user can interact with the LCD screen.
...
If the program is in recording mode (it is saving the audio signals into an audio file), the audio data should be sent to a web server for storage through the ESP32 device.

When the program is playing a saved audio file, the audio data shall be sent from the web to the AtMega328PB via the ESP32 chip, where its amplitude and frequency can be outputted to the speaker pin via a sine wave to play the audio.
...
The ESP32 chip shall be used to send and receive audio file data from the web. 
...
By the first milestone, we expect to have all the necessary components ordered. This includes the microphone, speaker, LED strips, an LCD screen, and other components like a knob for volume control and buttons for screen interaction. We also expect to have implemented real-time audio visualization and amplification by the first milestone. In other words, when music is played into the device's microphone, the device should be able to output the audio signals through the LED strips and the speaker. We also hope to have implemented adjustable volume control via a potentiometer knob. Also, we expect to have obtained and properly modified a container that can house all the circuitry and sensors as well as the knobs and buttons for volume control, recording music, selecting music files, etc..

### 7. Final Demo

What do you expect to achieve by the final demonstration or after milestone 1?

By the final demonstration, we expect have implemented the audio recording and file storage features. This should allow the user to press a button to start recording, play audio into the microphone of the device, and then stop recording and save the file. Then, through an LCD screen, the user should be able to select and play their desired music files through the speaker and LED light strips. Because the memory space on the AtMega328PB is limited, we will communicate with a web server via the ESP32 chip where the AtMega328PB can send audio data to and read audio data from. We also expect to have fitted all the components (MCU, speaker, microphone, LED strips, circuit parts) into the container with all the user interaction parts (knobs, sensors, and LED light strips) integrated for the user to easily use. 
...
The ESP32 chip used in Lab 4 will be utilized to allow the system to read and write audio files on a web server.
